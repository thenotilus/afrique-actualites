...
**Phase 8 (partage réseaux sociaux) — volet Facebook avancé avant lancement** :

- [x] Publication automatique sur la Page Facebook via l'API Graph (`App\Social\FacebookPublisher`)
- [x] Commande `app:articles:share` (§3.8) : un article par exécution, anti-répétition sur les
      mots-clés (fenêtre de 6h), cron toutes les 20 minutes
- [ ] `FACEBOOK_PAGE_ID` / `FACEBOOK_PAGE_ACCESS_TOKEN` à renseigner en production (`.env.local` ou
      secret Symfony) — voir `config/packages/social.yaml`
- [ ] Volet Twitter : toujours différé (interface d'extension `Social/` uniquement)
